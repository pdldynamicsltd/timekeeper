...
public class UserAppService_Update_Tests : UserAppServiceTestBase
{
    ...
    [Fact]
    public async Task Should_Clear_External_Login_Only_Claim_When_Admin_Sets_Password()
    {
        var user = await CreateExternalOnlyUserAsync("externalonlyuser");
        ...
        (await UserManager.IsExternalLoginOnlyAsync(updatedUser)).ShouldBeFalse();
    }
    ...
    private async Task<User> CreateExternalOnlyUserAsync(string userName)
    {
        var user = new User
        {
