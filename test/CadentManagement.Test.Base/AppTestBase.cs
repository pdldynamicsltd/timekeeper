...
using CadentManagement.Test.Base.TestData;
...
/// Provides methods to easily work with <see cref="CadentManagementDbContext"/>.
/// </summary>
public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
{
    ...
    {
        void NormalizeDbContext(CadentManagementDbContext context)
        {
    ...
    protected void UsingDbContext(Action<CadentManagementDbContext> action)
    {
        UsingDbContext(AbpSession.TenantId, action);
    }

    protected Task UsingDbContextAsync(Func<CadentManagementDbContext, Task> action)
    {
        return UsingDbContextAsync(AbpSession.TenantId, action);
    }

    protected TResult UsingDbContext<TResult>(Func<CadentManagementDbContext, TResult> func)
    {
        return UsingDbContext(AbpSession.TenantId, func);
    }

    protected Task<TResult> UsingDbContextAsync<TResult>(Func<CadentManagementDbContext, Task<TResult>> func)
    {
        return UsingDbContextAsync(AbpSession.TenantId, func);
    }

    protected void UsingDbContext(int? tenantId, Action<CadentManagementDbContext> action)
    {
        using (UsingTenantId(tenantId))
        {
            using (var context = LocalIocManager.Resolve<CadentManagementDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
    ...
    protected async Task UsingDbContextAsync(int? tenantId, Func<CadentManagementDbContext, Task> action)
    {
        using (UsingTenantId(tenantId))
        {
            using (var context = LocalIocManager.Resolve<CadentManagementDbContext>())
    ...
    protected TResult UsingDbContext<TResult>(int? tenantId, Func<CadentManagementDbContext, TResult> func)
    {
        TResult result;
    ...
    protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<CadentManagementDbContext, Task<TResult>> func)
    {
        TResult result;
