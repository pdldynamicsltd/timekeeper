...
test.describe('EDITIONS', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should hide error when form is properly filled', async () => {
            ...
            await editionsPage.triggerValidation('input[name="edition.displayName"]', '#MonthlyPrice', '#AnnualPrice');
        ...
        test('should save changes to record when "Save" button is clicked', async () => {
            await editionsPage.fillInputs({ 'input[name="edition.displayName"]': 'changed_name' });
            await editionsPage.saveModal();
            await editionsPage.waitForResponse();
        ...
        async function triggerValidation() {
            await fillInputsWithValidValues();
            await editionsPage.clearInputs('input[name="edition.displayName"]', '#MonthlyPrice', '#AnnualPrice');
        }

        function fillInputsWithValidValues() {
            return editionsPage.fillInputs({
                'input[name="edition.displayName"]': 'test',
                '#MonthlyPrice': '1',
                '#AnnualPrice': '2',
            });
        }
