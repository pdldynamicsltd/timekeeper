...
test.describe('VISUALSETTINGS', () => {
    ...
    test.describe('CODE FLOW', () => {
        ...
        async function changeTheme(page: Page, theme: string) {
            await page.locator('.theme-tab .theme-name').getByText(theme, { exact: true }).click();
            await page.waitForTimeout(1000);
            await page.locator('button:visible', { hasText: 'Save as system default' }).click();
            await visualSettingsPage.waitForResponse();
            await page.waitForTimeout(1000);
            await page.waitForSelector('.theme-select-box, .theme-tab', { state: 'visible' });
