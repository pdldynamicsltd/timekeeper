...
test.describe('USERS', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should display actions on click to "Actions" button', async () => {
            ...
            await page.waitForTimeout(500);
            await openActionsDropdownForUserName('test');
            await usersPage.wait(1000);
        ...
        async function openActionsDropdownForUserName(userName: string) {
            const row = page.locator(`.ant-table-tbody tr.ant-table-row`, { has: page.locator(`td:nth-child(2)`, { hasText: userName }) }).first();
