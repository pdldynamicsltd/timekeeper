...
test.describe('ORGANIZATIONUNIT', () => {
    ...
    test.describe('CRUD', () => {
        ...
        /* Step 7 */
        test('should display actions on click to right-click context menu', async () => {
            await organizationUnitPage.clickButtonByText('Cancel');
            await page.waitForFunction(() => !document.querySelector('.ant-modal-wrap:not([style*="display: none"])'));
            await page.waitForTimeout(500);
            await openOUContextMenu();
        ...
        test('should add sub organization', async () => {
            await openOUContextMenu();
            await clickContextMenuItem('Add sub-unit');
            await organizationUnitPage.waitForModal();
        ...
        async function openOUContextMenu() {
            const firstTreeNode = organizationUnitPage.page.locator('.ant-tree .ant-tree-node-content-wrapper').first();
            await firstTreeNode.waitFor({ state: 'visible' });
            await firstTreeNode.click({ button: 'right' });
            await organizationUnitPage.page.waitForSelector('.ant-dropdown:not(.ant-dropdown-hidden) .ant-dropdown-menu', { state: 'visible' });
        }

        async function clickContextMenuItem(text: string) {
            const visibleContextMenu = organizationUnitPage.page.locator('.ant-dropdown:not(.ant-dropdown-hidden) .ant-dropdown-menu').first();
            await visibleContextMenu.getByText(text, { exact: false }).first().click();
        }
    });
});
