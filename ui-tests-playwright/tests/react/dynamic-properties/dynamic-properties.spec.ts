...
test.describe('DYNAMICPROPERTIES', () => {
    ...
    test.describe('CRUD', () => {
        ...
        test('should save record when "Save" button is clicked', async () => {
            await dynamicPropertiesPage.fillInputs({ '#DynamicPropertyName': 'test' });
            await dynamicPropertiesPage.selectOptionByValue('#DynamicInputType', 'SINGLE_LINE_STRING');
            await dynamicPropertiesPage.saveModal();
        ...
        test('should display actions on click to "Actions" button', async () => {
            await page.waitForSelector('.ant-table-tbody tr.ant-table-row', { state: 'visible' });
        ...
        test('should save changes to record when "Save" button is clicked', async () => {
            await dynamicPropertiesPage.fillInputs({ '#DynamicPropertyName': 'changed_name' });
            await dynamicPropertiesPage.saveModal();
            await dynamicPropertiesPage.waitForResponse();
