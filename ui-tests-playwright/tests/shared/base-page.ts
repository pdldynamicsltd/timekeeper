...
export class BaseReactPage extends BasePage {
    ...
    async waitForConfirmationDialog(state: Visibility = 'visible') {
        await this.page.waitForSelector('.ant-modal-confirm', { state });
        await this.page.waitForTimeout(500);
    }

    async waitForDropdownMenu(state: Visibility = 'visible') {
        if (state === 'visible') {
            await this.page.waitForSelector('.ant-dropdown:not(.ant-dropdown-hidden)', { state: 'visible' });
        } else {
            // Wait for all dropdowns to be hidden or removed
    ...
    async waitForModal(state: Visibility = 'visible') {
        if (state === 'visible') {
            await this.page.waitForSelector('.ant-modal:not(.ant-modal-confirm)', { state: 'visible' });
        } else {
            await this.page.waitForFunction(() => {
    ...
    async waitForTableContent(tableSelector = '.ant-table', state: Visibility = 'visible') {
        await this.page.waitForSelector(`${tableSelector} .ant-table-tbody tr.ant-table-row`, { state });
        await this.page.waitForTimeout(500);
    }

    confirmConfirmation() {
        return this.page.click('.ant-modal-confirm .ant-btn-primary');
    }

    cancelConfirmation() {
        return this.page.click('.ant-modal-confirm .ant-btn-default');
    }

    async saveModal() {
        await this.page.click('.ant-modal:not(.ant-modal-confirm) .btn.btn-primary.fw-bold');
    }

    async openActionsDropdown(rowNumber = 1, tableCssSelector = '') {
        if (!tableCssSelector) {
            await this.page.click(`css=.ant-table-tbody >> xpath=//tr[${rowNumber}] >> text=/.*Actions.*/`);
        } else {
            await this.page.click(`css=${tableCssSelector} .ant-table-tbody >> xpath=//tr[${rowNumber}] >> text=/.*Actions.*/`);
        }
    }

    async triggerDropdownAction(actionText: string) {
        const visibleMenu = this.page.locator('.ant-dropdown:not(.ant-dropdown-hidden) .ant-dropdown-menu').first();
        await visibleMenu.waitFor({ state: 'visible' });

        const actionItem = visibleMenu.locator('.ant-dropdown-menu-item').filter({ hasText: actionText }).first();
        await actionItem.waitFor({ state: 'visible' });
        await actionItem.click();

        const menuStillVisible = await visibleMenu.isVisible().catch(() => false);
        if (menuStillVisible) {
