...
namespace CadentManagement.Authorization.Accounts;

public class AccountAppService : CadentManagementAppServiceBase, IAccountAppService
{
    ...
    public async Task SendPasswordResetCode(SendPasswordResetCodeInput input)
    {
        ...
        user.PasswordResetCodeExpireDate = Clock.Now.AddHours(expirationHours);
    ...
    [EnableRateLimiting("PasswordResetLimiter")]
    public async Task<ResetPasswordOutput> ResetPassword(ResetPasswordInput input)
    {
        ...
        if (user.IsLockoutEnabled && user.LockoutEndDateUtc.HasValue && user.LockoutEndDateUtc > Clock.Now.ToUniversalTime())
        {
            throw new UserFriendlyException(L("UserLockedOutMessage"));
        }

        // Enforce server-side reset code expiry
        ...
        // Validate reset code and apply lockout on failure
        if (user.PasswordResetCode != input.ResetCode)
        {
        ...
        await UserManager.ClearExternalLoginOnlyClaimAsync(user);
        await UserManager.UpdateAsync(user);

        await _userEmailer.SendPasswordChangedNotificationAsync(user);
    ...
    private async Task SendSmsPasswordlessCode(string phoneNumber)
    {
        ...
        var message = string.Format(L("PasswordlessLogin_SmsMessage", CadentManagementConsts.ProductName, code));
