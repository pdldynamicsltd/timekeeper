...
public class StripeGatewayManager : CadentManagementServiceBase,
    ...
    ITransientDependency
{
    ...
    public async Task UpdateSubscription(SubscriptionPayment payment, bool isProrateCharged = false)
    {
        ...
        if (!(await DoesPlanExistAsync(newPlanId)))
        {
            await CreatePlanAsync(
                ...
                CadentManagementConsts.ProductName
            );
        }
    ...
    public async Task HandleInvoicePaymentSucceededAsync(Invoice invoice)
    {
        ...
        foreach (var i in invoice.Lines.Data)
        {
            var item = new RecurringPaymentLineItem
            {
                ...
                PlanId = i.Pricing?.PriceDetails?.PriceId,
                PlanType = await GetPlanTypeByIdAsync(i.Pricing?.PriceDetails?.PriceId),
    ...
    public async Task<StripeIdResponse> GetOrCreatePlanForPayment(long paymentId)
    {
        ...
        var product = await GetOrCreateProductAsync(CadentManagementConsts.ProductName);
