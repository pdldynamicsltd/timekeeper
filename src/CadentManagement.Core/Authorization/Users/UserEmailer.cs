...
/// </summary>
public class UserEmailer : CadentManagementServiceBase, IUserEmailer, ITransientDependency
{
    ...
    public async Task SendPasswordChangedNotificationAsync(User user)
    {
        ...
        {
            var tenancyName = GetTenancyNameOrNull(user.TenantId);
            var emailTemplate = GetTitleAndSubTitle(user.TenantId, L("PasswordChangedEmail_Title"),
                L("PasswordChangedEmail_SubTitle"));
            var mailMessage = new StringBuilder();

            mailMessage.AppendLine("<b>" + L("NameSurname") + "</b>: " + WebUtility.HtmlEncode(user.Name) + " " + WebUtility.HtmlEncode(user.Surname) + "<br />");

            if (!tenancyName.IsNullOrEmpty())
            {
                mailMessage.AppendLine("<b>" + L("TenancyName") + "</b>: " + WebUtility.HtmlEncode(tenancyName) + "<br />");
            }

            mailMessage.AppendLine("<b>" + L("UserName") + "</b>: " + WebUtility.HtmlEncode(user.UserName) + "<br />");
            mailMessage.AppendLine("<br />");
            mailMessage.AppendLine(string.Format(L("PasswordChangedEmail_Body"),
                Clock.Now.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture) + " UTC") + "<br />");

            await ReplaceBodyAndSendAsync(user.EmailAddress, L("PasswordChangedEmail_Subject"), emailTemplate,
                mailMessage);
        }
