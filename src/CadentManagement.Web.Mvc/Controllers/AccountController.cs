...
using CadentManagement.Authorization.Impersonation;
...
using CadentManagement.Net.Sms;
using CadentManagement.Notifications;
using CadentManagement.Web.Models.Account;
...
using CadentManagement.Web.Authentication.External;
using CadentManagement.Web.Security.Recaptcha;
using CadentManagement.Web.Session;
using CadentManagement.Web.Views.Shared.Components.TenantChange;
...
using CadentManagement.Authentication.PasswordlessLogin;
using CadentManagement.Authorization.PasswordlessLogin;
using CadentManagement.Authorization.QrLogin;
using CadentManagement.Authorization.Session;
using CadentManagement.Web.Authentication.JwtBearer;

namespace CadentManagement.Web.Controllers;

public class AccountController : CadentManagementControllerBase
{
        ...
        string returnUrlHash = "", string ss = "")
    {
        ...
        if (loginResult.User.ShouldChangePasswordOnNextLogin)
        {
            ...
            var expirationHours = _settingManager.GetSettingValue<int>(
                AppSettings.UserManagement.Password.PasswordResetCodeExpirationHours
            );
            loginResult.User.PasswordResetCodeExpireDate = Clock.Now.AddHours(expirationHours);

            var code =
                $"?userId={loginResult.User.Id}&resetCode={loginResult.User.PasswordResetCode}";
        ...
        if (sessionToken != null)
        {
            ...
            await _signInManager.SignInWithClaimsAsync(loginResult.User, loginModel.RememberMe, additionalClaims);
        }
    ...
    {
        // Invalidate user session record before signing out
        if (await _userSessionManager.IsSessionManagementEnabledAsync())
        {
            var sessionTokenClaim = HttpContext.User?.Claims.FirstOrDefault(c => c.Type == AppConsts.SessionTokenKey);
    ...
    public virtual async Task<JsonResult> VerifySecurityCode(VerifySecurityCodeViewModel model)
    {
        ...
        if (signInResultResult.Succeeded)
        {
            await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
                ...
                var sessionToken = await CreateUserSessionIfEnabledAsync(user.Id, user.TenantId);
                if (sessionToken != null)
                {
                    var additionalClaims = new List<Claim>
                    {
    ...
    public virtual async Task<JsonResult> VerifyPasswordlessCode(VerifyPasswordlessCodeViewModel model)
    {
        ...
        await SignInWithSessionAsync(user, false);
    ...
    public async Task<ActionResult> Register(RegisterViewModel model)
    {
        try
        {
            ...
            if (model.IsExternalLogin)
            {
                ...
                await _userManager.AddClaimAsync(user, new Claim(ExternalLoginConsts.ExternalLoginOnlyClaimType, "true"));
            }
    ...
    public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
    {
        ...
        // Enforce server-side reset code expiry (treat null as expired to fail closed for pre-migration codes)
        if (!user.PasswordResetCodeExpireDate.HasValue || user.PasswordResetCodeExpireDate < Clock.Now)
        {
    ...
    public async Task<ActionResult> ResetPassword(ResetPasswordInput input,
        string returnUrl = null, string singleSignIn = null)
    {
        ...
        if (output.CanLogin)
        {
            ...
            if (!string.IsNullOrEmpty(singleSignIn) &&
                singleSignIn.Equals("true", StringComparison.OrdinalIgnoreCase))
            {
                user.SetSignInToken();
                var redirectUrl =
                    AddSingleSignInParametersToReturnUrl(returnUrl, user.SignInToken, user.Id, user.TenantId);
        ...
        string ss = "")
    {
        ...
        switch (loginResult.Result)
        {
            case AbpLoginResultType.Success:
                {
                    await SignInWithSessionAsync(loginResult.Identity, false);
    ...
    #region External Login Link (server-side Challenge flow)

    [AbpMvcAuthorize]
    [HttpGet]
    public ActionResult ExternalLoginLink(string provider)
    {
        var redirectUrl = Url.Action(
            "ExternalLoginLinkCallback",
            "Account",
            new { provider });
    ...
    [AbpMvcAuthorize]
    public virtual async Task<ActionResult> ExternalLoginLinkCallback(string provider, string remoteError = null)
    {
        var profileUrl = Url.Action("Index", "Home", new { area = "App" });

        if (remoteError != null)
        {
            Logger.Error("Remote Error in ExternalLoginLinkCallback: " + remoteError);
            return Redirect(profileUrl + "?linkResult=error&linkProvider=" + provider);
        }

        var externalLoginInfo = await _signInManager.GetExternalLoginInfoAsync();
        if (externalLoginInfo == null)
        {
            Logger.Warn("Could not get information from external login for linking.");
            return Redirect(profileUrl + "?linkResult=error&linkProvider=" + provider);
        }

        var providerKey = externalLoginInfo.ProviderKey;
        var loginProvider = externalLoginInfo.LoginProvider;
        var emailAddress = GetEmailAddressFromExternalLoginInfo(externalLoginInfo);
        ...
        try
        {
            ...
            if (result.Success)
            {
                return Redirect(profileUrl + "?linkResult=success&linkProvider=" + loginProvider);
            }

            if (result.ProviderAlreadyLinkedToAnotherUser && result.CanMerge)
            {
                ...
                return Redirect(profileUrl + "?linkResult=merge&linkProvider=" + loginProvider +
                    "&linkMergeToken=" + Uri.EscapeDataString(mergeToken) +
                    "&linkExistingEmail=" + Uri.EscapeDataString(result.ExistingUserEmail ?? ""));
            }

            return Redirect(profileUrl + "?linkResult=alreadyLinked&linkProvider=" + loginProvider);
        }
        catch (Exception ex)
        {
            Logger.Error("Error linking external login", ex);
            return Redirect(profileUrl + "?linkResult=error&linkProvider=" + loginProvider);
        }
    }

    [AbpMvcAuthorize]
    [HttpPost]
    public async Task<JsonResult> MergeAndLinkExternalLoginServerSide([FromBody] ServerSideMergeExternalLoginInput input)
    {
    ...
    private static string GetEmailAddressFromExternalLoginInfo(ExternalLoginInfo externalLoginInfo)
    {
        return externalLoginInfo.Principal.FindFirstValue(ClaimTypes.Email) ??
               ...
               externalLoginInfo.Principal.FindFirstValue("upn");
    }
    ...
    public virtual async Task<ActionResult> ImpersonateSignIn(string tokenId)
    {
        ...
        await SignInWithSessionAsync(result.Identity, false);
    ...
    private async Task SignInWithSessionAsync(User user, bool isPersistent)
    {
        var sessionToken = await CreateUserSessionIfEnabledAsync(user.Id, user.TenantId);
    ...
    private async Task SignInWithSessionAsync(ClaimsIdentity identity, bool isPersistent)
    {
        var userIdClaim = identity.FindFirst(c => c.Type == AbpClaimTypes.UserId);
        var tenantIdClaim = identity.FindFirst(c => c.Type == AbpClaimTypes.TenantId);

        if (userIdClaim != null)
        {
            var userId = long.Parse(userIdClaim.Value);
            int? tenantId = tenantIdClaim != null ? int.Parse(tenantIdClaim.Value) : null;
            var sessionToken = await CreateUserSessionIfEnabledAsync(userId, tenantId);

            if (sessionToken != null)
            {
                identity.AddClaim(new Claim(AppConsts.SessionTokenKey, sessionToken));
            }
        }

        await _signInManager.SignInAsync(identity, isPersistent);
    }
